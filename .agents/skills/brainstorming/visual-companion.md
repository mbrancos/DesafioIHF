...
```powershell
# In Antigravity IDE, launch using PowerShell:
.\.agents\skills\brainstorming\scripts\start-server.ps1 -ProjectDir . -Open

# To stop the server when brainstorming finishes:
.\.agents\skills\brainstorming\scripts\stop-server.ps1 -ProjectDir .
```
*(Tip: In Google Antigravity IDE, you also have access to `generate_image` for generating visual mockup assets directly into artifacts, and `browser_subagent` for browser automation.)*
